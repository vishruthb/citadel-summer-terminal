...
class GameState:
    ...
    def attempt_spawn(self, unit_type, locations, num=1, save = False):
        ...
        for location in locations:
            for i in range(num):
                if save and self.get_resource(self.SP) == 1:
                    return spawned_units
                if self.can_spawn(unit_type, location, 1):
                    x, y = map(int, location)
                    costs = self.type_cost(unit_type)

                    #if self.get_resource(self.SP) - costs[0] == 1 and unit_type == WALL:
                        #print(self.SP, costs[0])
                        #return spawned_units
